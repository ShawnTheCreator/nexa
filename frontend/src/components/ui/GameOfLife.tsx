...
const GameOfLife = () => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
          ...
          if (cell.opacity > 0) {
            ctx.fillStyle = `rgba(147, 51, 234, ${cell.opacity})` // Purple color (Tailwind's purple-600)
            ctx.beginPath()
            ctx.arc(
  ...
  return (
    <div className="pointer-events-none select-none overflow-hidden mask bg-black/50">
      <canvas ref={canvasRef} width={1500} height={600} className="mix-blend-screen" />
    </div>
  )
}
